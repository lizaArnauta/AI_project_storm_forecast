...
This project uses the NHITS (Neural Hierarchical Interpolation for Time Series) architecture to forecast wave heights 96 hours (4 days) ahead for a point in the North Sea (56.0°N, 3.0°E). By combining weather data (pressure, wind speed) with historical wave measurements, the model achieves prediction accuracy that could be useful for maritime operations.

**Best Model Performance: MAE 0.325m** (Huber Loss with rolling wind features)
...
Simple NHITS with basic features (pressure, wind speed).
- **Input size:** 48 hours
- **Horizon:** 96 hours
- **Loss function:** MAE
- **Training steps:** 2000
...
### 3. Physics-Informed Features + Ensemble (MAE: 0.334m)
...
*Note: Smoothed model's 0.327m is measured against smoothed test data. Against original data it's 0.381m.
...
- [ ] Incorporate wave period and direction
- [ ] Implement probabilistic forecasting (quantile predictions for uncertainty)
- [ ] Experiment with longer horizons (7-day forecasts)

## Why NHITS?

NHITS (Neural Hierarchical Interpolation for Time Series) is perfect for this application because:
- **Multi-scale patterns:** Captures both short-term wind fluctuations and longer pressure systems
- **Efficient:** Only 2.9-3.0M parameters compared to transformers
- **Fast inference:** Critical for real-time marine forecasting
- **Robust:** Handles missing data and irregular patterns well
---
